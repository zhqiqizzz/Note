...
1. 当前流式题目主要使用累计文本覆盖，短文本足够稳定；长报告将改为 `delta + sequence`，降低重复传输和乱序风险。
    
2. SSE 可以补充心跳事件，防止 Nginx 或网关关闭长时间无数据连接。
...
4. 复杂 Markdown 或长流式文本可做批量渲染，避免每个 token 都触发昂贵 DOM 更新。
...
```
{
  "errorCode": "INTERVIEW_AI_TIMEOUT",
  "retryable": true
}
```
