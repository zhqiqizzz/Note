...
    participant App as Vue App 实例

    participant Plugin as 插件对象

  

    User->>App: app.use(Plugin, { theme: 'dark' })

    Note over App: 1. 检查 _plugins 数组<br/>是否包含 Plugin?
...
        Note over App: 2. 判断 Plugin 类型

        alt 是函数

            App->>Plugin: Plugin(app, { theme: 'dark' })

        else 是对象

            App->>Plugin: Plugin.install(app, { theme: 'dark' })

        end

        Note over Plugin: 执行安装逻辑:<br/>- 注册组件<br/>- 注入 Provide<br/>- 修改 globalProperties

        Plugin-->>App: 执行完毕

        Note over App: 3. 将 Plugin 推入 _plugins 数组

        App-->>User: 返回 app (支持链式调用)
